...
def generate_reachable_target(
    ...
    modes = ["random", "inside_drawer"]
    mode = random.choice(modes)
    if mode == "random":
        rdata = rmodel.createData()
        q = generate_collision_free_configuration(rmodel, cmodel, max_tries)
        pin.framesForwardKinematics(rmodel, rdata, q)
        target_pose = rdata.oMf[rmodel.getFrameId("panda_hand_tcp")]
    else: # inside_drawer
        x = np.random.uniform(0.45, 0.8)
        y = np.random.uniform(-0.25, 0.25)
        z = np.random.uniform(0.05, 0.70)
        position = np.array([x, y, z])
        R = np.eye(3) # We don't really care about the orientation here
        target_pose = pin.SE3(R, position)
    return target_pose
