...
from conditional_diffusion_motion.utils.panda.create_drawer import DrawerEnv
...
# Configuration
# -----------------------------
OUTPUT_DIR = Path("/home/arthur/Desktop/Code/slot_attention_diffusion/ressources/drawer_example/generated_scenes/drawer_scenes")
...
def generate_shelf_only_scene(scene_idx: int):
    ...
    image.save(OUTPUT_DIR / f"drawer_scene_{scene_idx:04d}.png")
...
if __name__ == "__main__":
    ...
    with progress:
        task = progress.add_task("Drawer scenes", total=N_SCENES)
        for idx in range(N_SCENES):
            generate_shelf_only_scene(idx)
            progress.advance(task)

    print(f"\n✅ Saved {N_SCENES} drawer scenes to: {OUTPUT_DIR.resolve()}")
