...
class TrajectoryPredictorWithCspace:
    def __init__(
        self,
        diffusion_ckpt_path,
        device=None,
        ...
        self.model = ModelCSpace.load_from_checkpoint(diffusion_ckpt_path).to(self.device)
        self.model.eval()
    def predict(
        self,
        initial_pose,  # shape: (nq,), torch.Tensor or np.ndarray
        goal,  # shape: (3,), torch.Tensor or np.ndarray
        validity_mask: torch.Tensor = None,  # C-space validity mask
        seq_length=15,
        ...
        cond = {
            ...
            "cspace": validity_mask.unsqueeze(0).repeat(bs, 1),  # (bs, cspace_size)
        }
