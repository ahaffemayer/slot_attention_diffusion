...
class IndirectMotionDatasetWithObstaclesWithDrawerSlotAttention(Dataset):
    def __init__(self, slot_encoder, data_dir: Path, device="cuda", target_total_trajectories = None):
        ...
        self.image_dir = self.data_dir / "generated_scenes" / "drawer_for_diffusion"

        raw_file = self.data_dir / "trajectories" / "trajectories_data_drawer.json"
        cache_dir = self.data_dir / "processed_datasets"
        cache_dir.mkdir(exist_ok=True)

        file_samples = cache_dir / "indirect_motion_with_obstacles_cache_samples_with_drawer.npy"
        file_q0 = cache_dir / "indirect_motion_with_obstacles_cache_q0_with_drawer.npy"
        file_goal = cache_dir / "indirect_motion_with_obstacles_cache_goal_with_drawer.npy"
        file_scenes = cache_dir / "indirect_motion_with_obstacles_cache_scene_ids_with_drawer.npy"
        file_slots = cache_dir / "indirect_motion_with_obstacles_cache_slots_with_drawer_slot_attention.npy"
...
class IndirectDataModuleWithObstacleWithDrawerSlotAttention(pl.LightningDataModule):
    ...
    def __init__(self, data_dir: Path = None, slot_attention_model_dir: Path = None, target_total_trajectories = None) -> None:
        ...
        self.dataset = IndirectMotionDatasetWithObstaclesWithDrawerSlotAttention(
