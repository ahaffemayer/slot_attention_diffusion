...
class IndirectMotionDatasetWithObstaclesWithShelfOccupancyGrid(Dataset):
    def __init__(self, data_dir: Path, device="cuda"):
        """
        Dataset that returns robot motion data with associated occupancy grid features from scene images.
        ...
        self.image_dir = (
            self.data_dir / "generated_scenes" / "shelf_for_diffusion"
        )
        ...
        file_q0 = (
            cache_dir
            / "indirect_motion_with_obstacles_cache_q0_with_shelf.npy"
        )
        file_goal = (
            cache_dir
            / "indirect_motion_with_obstacles_cache_goal_with_shel.npy"
        )
        file_scenes = (
            cache_dir
            / "indirect_motion_with_obstacles_cache_scene_ids_with_shelf.npy"
        )
        file_occupancy_grid = (
            cache_dir
            / "indirect_motion_with_obstacles_cache_with_shelf_occupancy_grid.npy"
        )
        ...
        if os.path.exists(file_occupancy_grid):
            self.occupancy_grid = torch.tensor(np.load(file_occupancy_grid), dtype=torch.float32)
        else:
            raise FileNotFoundError(
                f"File {file_occupancy_grid} not found. Please generate the Occupancy Grid data first."
            )
    ...
    def __getitem__(self, idx):
        ...
        occupancy_grid_feat = self.occupancy_grid[scene_id]
        return self.samples[idx], {
            "q0": self.q0[idx],
            "goal": self.goal[idx],
            "occupancy_grid": occupancy_grid_feat,
        }


class IndirectDataModuleWithObstacleWithShelfOccupancyGrid(pl.LightningDataModule):
    """Defines the data used for training the diffusion process."""

    def __init__(self, data_dir:Path = None) -> None:
        super().__init__()

        self.dataset = IndirectMotionDatasetWithObstaclesWithShelfOccupancyGrid(
